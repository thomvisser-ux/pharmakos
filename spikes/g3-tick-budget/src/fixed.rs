...
//!
//! Copied from the G4 spike's `src/fixed.rs` unchanged except for this note and
//! the coordinate bound in [`Sq::between`]'s doc comment (G3' uses a 384x384x64
//! footprint rather than G4's +/-1_024 box). `spikes/README.md` rule 2 permits
//! copying between spikes; the point of copying rather than rewriting is that
//! the two spikes' hashes are then comparable, which is what makes this spike's
//! per-tick hash stream free G4 evidence.
//!
...
//!
//! * no floating point of either width — the sine table is built by an integer
//!   Taylor series in
...
impl Sq {
    ...
    ///
    /// Bound: the toy map clamps every coordinate into `[0, 384]` voxels
    /// horizontally and `[0, 64]` vertically, so each delta is at most 2^25
    /// raw, each square at most 2^50, and the sum of three at most 2^51.6 —
    /// comfortably inside `i64` with overflow checks on.
    #[must_use]
