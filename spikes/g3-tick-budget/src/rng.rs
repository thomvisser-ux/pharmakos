...
pub enum Stream {
    ...
    Spawn,
    Map,
    /// Unit and building programs (stance flips, build jitter).
    Program,
    /// The every-5th-tick playbook/mandate decision pass.
    Decision,
    /// Destruction: crater centres and radii.
    Voxel,
    /// **Cost model only.** The repath-demand burst draw of the `pathing`
    /// phase. Deliberately a stream of its own so that the synthetic pathing
    /// load — which is state-free and whose size depends on a wall-clock
    /// calibration — cannot perturb a draw that reaches hashed state. See
    /// `tick.rs`'s `phase_pathing`.
    PathCost,
}
