...
## Build state (2026-09-13, paused for compact)
...
- **Spike G1 (mesher/remesh in Godot 4.7.2 via gdext) is RUNNING** as workflow `spike-g1-remesh` (run id `wf_e6ebfb81-bc9`, task `w94yr8vmm`; phases Build → 3 reviews → Fix → Measure). Its output lands at `…\697d68ef-4644-4c21-b529-4b881d0f7eea\tasks\w94yr8vmm.output` and the journal under `C:\Users\PC\.claude\projects\C--Users-PC-pharmakos\697d68ef-4644-4c21-b529-4b881d0f7eea\subagents\workflows\wf_e6ebfb81-bc9\journal.jsonl`. It writes only under `spikes/g1-remesh/` and `docs/spikes/G1-destruction-remesh.md` §10; nothing of it is committed yet.

## NEXT
1. When the G1 workflow reports: read its result, apply any repo-level fixes it could not make (it was path-restricted), commit `spikes/g1-remesh` + the plan, push, record the G1 verdict and its decision (upload path A/B, budget K/B) in the decisions log, and interview the owner on that decision.
