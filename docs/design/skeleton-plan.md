...
- **Amended 2026-09-21 (items 106 and 107): this section is not buildable as written above.** *Needs* gains **T16a** and T15 merged; terrain and entities come from `get_view`, never from the sim; speed and skip are a walled `Pacer` asking `advance_push` for game milliseconds; the lobby spawns `gamectl host` and holds two connections; the two hash-chain tests moved to T16a and pacer, fixture and live-host tests replace them; run 2 grants the named place `crates/gamectl/src/host.rs`. The amendment in full, with the notes for T15, T17, T18, T19, T21, T22 and S7, is `docs/design/skeleton-plan-t16a-notes.md`, and it outranks the lines above where they disagree.

---

### T16a — `proto/gp/api/v1` + `crates/gateway` part 3: the view feed, match control and the host loop

- **Crate(s) owned:** `proto/gp/api/v1/gateway.proto`, the regenerated tree and descriptor set in `crates/proto` plus one new module there (`chunk_rle`), the schema goldens under `tests/golden/proto/`, and `crates/gateway` **except** T14's three named places (item 104(2): `strings::event_text`, `Surface::audience_of`, the rows of `two_door_cases()`). **No edit in `crates/sim`, none in `crates/gamectl`, none in `crates/client-gdext`.** Runs as the third lane beside T14 and T15. `Surface::{new, attach, open_lull, call, begin_push, step, end_recap}` and `TickReport` keep their signatures (T15 builds on them).
- **Builds:** the three things T16 presupposes and `main` does not have (item 106), cut to what demo 1.1 needs.
  **(1) The view feed** — one read method, `get_view` = 71 (`observe`, every phase): `GetViewRequest { cursor }`, `GetViewResponse { at_ms, chunks, entities, next_cursor, complete }`, `ViewChunk { origin, voxels_rle }`, `ViewEntity { id, kind (UNIT | BEACON | STRUCTURE), subtype, owner, at }`, numbers above each reserved to 15. An empty cursor is a keyframe; any other means "what changed since the view this cursor came from", so paging and deltas are one opaque, match-tied string rendered by `feed.rs`'s existing conventions under a new `Listing::View`; a cursor from before an `attach` is `STALE_SNAPSHOT`. Terrain is materials only, run-length encoded in the sim's voxel order as 3-byte runs; the codec lives once, in `pharmakos_proto::chunk_rle`, and **the material table and the voxel order are written into the proto comment**, with the rule that a decoder draws nothing for a value it does not know. **The generated map is served whole to every viewer (item 107(1)); what is fogged is what changes it and what stands on it, voxel-exactly:** for a modified chunk a viewer is entitled to the current byte where it is unfogged or `Vision` sees that voxel and to the generated byte elsewhere, and a chunk is in a delta **iff its bytes for that viewer changed since the cursor** — so an edit a seat cannot see produces no chunk at all, and the full-map unlock is nothing but the viewer becoming unfogged. Behind it a **derived, unhashed** `viewfeed.rs`: the generated map encoded once at `attach`; `changed_at[chunk]` stamped from `VoxelStore::settled()` after every single tick inside `Surface::step`; a monotonic `view_seq` bumped per stepped tick and per attach, phase change and policy change (**never a sim tick** — a Lull and a recap consume none); and per viewer the entitled bytes of modified chunks with the `view_seq` they last changed at, refreshed inside the `get_view` call, which is the only place a `Vision` exists. Entities are the **complete visible list on every completing page** (the comment says so, and that a delta mode is opt-in through a future request field), filtered row by row through the existing `FogFilter` and built by inclusion: what a viewer may not see is absent, and no total is reported. `id` is opaque: beacons keep `b_NN`; units and structures get **viewer-scoped handles in order of first sighting**, because a dense row id is a count. `owner` is spelt `seat.0`, empty for nobody. `at_ms` has `Event.at_ms`'s epoch. **Never on the wire:** a destination, a route, a step or rule index, a mandate, a treasury, a score, a hit point, a power state, a count. Pages are cut at 256 KiB of encoded runs, ascending chunk order, first chunk always through. The production `Vision` is `host::SphereVision` — own, alive beacon spheres at `beacon.sphere_radius_voxels`, the one sight rule the spec states *and* the table numbers — labelled a stopgap there and in `fog.rs`. `FogPolicy::eliminate` is wired from `SeatTable::is_alive` in `Surface::step` (it has no caller on `main`).
  **(2) Match control** — four `admin` methods in a new `surface/control.rs`: `end_lull` = 90; `advance_push { ms }` = 91 (**game milliseconds**, 1 to 60 000, floored to whole ticks inside the gateway, stops at segment end, answers `advanced_ms` and nothing else — segment and match end are the `_status` footer's phase; no tick and no state hash reaches the wire); `end_recap` = 92; `report_host_clock { elapsed_ms, remaining_ms }` = 93, valid in Lull, Recap and Ended, where `elapsed_ms` — monotone within a phase, at most 60 000 ms per report, anything else refused — is what moves the gateway's tick in every phase that consumes no sim tick (so the rate budget does not freeze in a recap or after match end), `remaining_ms` is what `_status` shows in a Lull, and the answer carries `all_ready`, the one thing the lobby needs to let Ready end the Lull. Speed is how much game time the client asks for; skip is `advance_push {60 000}` repeated until the footer leaves `PUSH`; the one wall clock stays in the walled client. Out of phase is `PHASE_CLOSED`; an out-of-range argument is `INVALID_ARGUMENT`, refused and never clamped. Every call goes through the same seven steps as any other. **This changes the confinement rule and says so:** no handler may step or seal, *except* the admin-scoped control handlers in `surface/control.rs`, which may drive only the live match through `Surface`'s three driving methods and may name none of `Runner`, `Host`, `World`, `host_mut`, `seal_plans`, `seal_playbook`, `snapshot`, `.clone()`.
  **(3) The host loop** — `serve.rs`: `serve::run(setup, control, announce)` with `Setup { rules_json, library, built_in }`. It reads **one** config line (match id, seed, seats, the human seat or none, segment lengths, round limit — no path: the binary reads the rules file and hands over its text, so `std::fs` stays in `cache.rs`), opens the `Host` and `Surface`, binds `Listener::bind(0)`, mints the admin token and the human seat's, writes **one** announce line (port, match id, the two rendered tokens), then serves connections against the one `Surface`: a reader thread per connection doing handshake and framing, **one request in flight per connection**, a bounded `sync_channel`, one thread that owns the surface, at most `MAX_CONNECTIONS` live (an audited 503 beyond it), and returns when `control` reaches end of file. After every `open_lull` it hands each `BuiltInSeat` a `call` closure bound to that seat's own token, so a built-in seat comes through `Surface::call` like any socket client; the list is empty here. `session.rs` splits into a transport half and a surface half. The module doc carries the argument for why answering in arrival order is safe (AGENTS.md §4.6). No binary: `gamectl host` is a named place in run 2. Because `Host::open` takes the sim’s `WorldConfig` and `gamectl` has no edge to the sim, the task also adds `Host::open_from(rules_json, seed, seats, settings)` — text and plain values in, parsed inside the gateway — which `serve.rs` uses itself and `gamectl scenario run` moves onto at T15’s rebase (item 107(11)).
  **Test seam, host-side only:** `Host::file_voxel_edit` and `Host::file_damage`, forwarding to `Runner::world_mut` as its doc describes; banned to every handler by `tests/confinement.rs`; reached by no wire method.
- **Implements:** spec section 3 (During play; Fog policy; "The human alone decides speed-up and skip"), section 12 (admin covers lobby and match control; fog filtered server-side; rate limits and the audit log as part of the feature), section 15 (Hosting; the client as a JSON-RPC client; Replays' "keyframes plus state deltas and events, with no playbooks" as the shape the feed is cut to); items 26, 54 (first-through paging), 92, 99, 106, 107; AGENTS.md §3 rules 2 and 4, §4.5, §4.6, §7.
- **Needs:** T13 (merged), and item 107(1), taken on the recommendation under the owner’s standing instruction and open to the owner’s overrule — the wire shape is the same under either answer, and switching costs about 1.5 agent-days. Nothing from T14 or T15. *Sequenced after T14 merges, as a follow-up and not as part of this task:* `SphereVision` is replaced by a sim query (`World::in_own_sphere`, reusing the sim's own `within`) and gains Survey-lite's sightings.
- **Acceptance:** determinism, read in process from `TickReport::hash` and **moved here from T16** — `advancing_in_steps_of_50_100_200_and_60000_ms_yields_the_same_chain`, `skipping_to_segment_end_yields_the_same_terminal_hash_as_playing_it_out`. View — `an_edit_a_seat_cannot_see_never_reaches_it_as_a_delta` (exact: one voxel outside the sphere, inside a half-seen chunk), `an_edit_a_seat_can_see_arrives_with_its_current_bytes`, `a_fogged_seat_never_receives_an_unseen_entity`, `an_entity_id_reveals_nothing_about_the_tables_size`, `no_view_ever_carries_a_destination_a_step_a_mandate_or_a_hit_point` (key-set allow-list, every viewer, both policies), `a_nofog_spectator_sees_every_edit_and_entity_and_never_a_playbook_draft_or_notebook`, `admin_gets_an_empty_view`, `a_view_response_reports_no_count_of_what_it_withheld`, `a_cursor_taken_in_a_lull_still_delivers_what_the_push_reveals`, `the_full_map_unlock_comes_from_a_server_side_policy_change` (elimination and match end, same token before and after, **under the default `Limits`** with the rig's other end-of-match calls), `the_wire_material_table_is_the_sims`, the `chunk_rle` round trip and its refusal of a short or long run list. Control — `only_an_admin_token_can_end_a_lull_advance_a_push_or_end_a_recap`, `a_control_method_out_of_phase_is_phase_closed_and_changes_nothing`, `an_advance_outside_1_to_60000_ms_is_refused_not_clamped`, `a_host_clock_that_runs_backwards_or_jumps_is_refused_not_clamped`, `the_host_clock_moves_the_rate_budget_in_a_recap_and_after_match_end`, `all_ready_reaches_the_admin_and_nothing_else_about_a_seat_does`, `every_control_handler_needs_the_admin_scope_in_the_schema`, `the_match_is_stepped_in_one_module` rewritten to the rule above, `no_wire_method_files_a_voxel_edit_or_a_damage_order`. Host — `two_connections_share_one_surface`, `a_built_in_seat_calls_through_the_same_door_as_a_socket`, `a_flood_of_silent_connections_cannot_disturb_a_live_session`, `the_host_exits_when_its_control_pipe_closes`, `the_host_binds_loopback_on_an_ephemeral_port`, `a_token_appears_on_the_announce_line_and_nowhere_else_the_host_writes`. Goldens under `tests/golden/gateway/`: `view_one_tick/expected.view.txt`; `view_keyframe/expected.keyframe.jsonl` — one served response per line — with its readable twin `expected.keyframe.txt` (**the fixture T16's hostless screenshot and client-gdext's tests load**); `walkthrough_watch/expected.watch.txt`; the spec's 14-call walkthrough is not touched. `expected.method-scopes.txt` and `expected.reserved.txt` re-blessed with the reason (`get_schema/expected.json` does not move: it serves `gp.v1` only). The measured encoded size of the whole-map keyframe for the golden seed, **reported as a number, not gated**. `buf breaking` green in `WIRE_JSON` mode; `cargo xtask ci` green.
- **Contract PR:** **yes** — `proto/**`, the generated tree and the schema goldens, as the **first commit** and alone in it; the agent opens the PR and stops. Additive only: five enum values and twelve messages, nothing renumbered. Reviewed as a contract beyond its paths, because it **changes** the rule `tests/confinement.rs` holds and adds the first production `Vision`. The PR asks for, and does not make, four harness-doc edits: AGENTS.md §3 rule 2 naming the match host and its control handlers (T13's open request), the `gamectl` row gaining `host`, the `gamectl` row gaining `pharmakos-operator` (or the owner's other home for a hosted built-in seat), and the `proto` row gaining the voxel run-length codec.
- **Agent-days:** 10. **One pull request by default** (item 107(9): one review cycle, because token spend comes first). **Split seam, taken only if the lane runs out of room:** PR 1 — proto, codec, view feed, vision, control, the test seam (7 d); PR 2 — `serve.rs`, the session split, the bounded loop, the built-in seat seam (3 d). T16's vista half needs PR 1 only.
- **PLACEHOLDERs:** generated terrain served whole, terrain fog's unit and memory (item 107(1); S3’s knowledge store, one mask feeding the view and the estimator); unit, structure, Survey and Spire vision and reach memory N — spheres only until then (owner; S1 with Survey-lite's sightings, S3); what a seat sees of an enemy asset beyond where it is — HP, power state, heading, build progress, numbers 6 to 15 reserved (owner, S2); `MAX_ADVANCE_MS` and the clock step = 60 000, `VIEW_PAGE_BYTES` = 256 KiB, `MAX_CONNECTIONS` = 8 (owner, hardening, with the rate limits, `MAX_MESSAGE_BYTES` and `READ_TIMEOUT`); whole-voxel positions and full entity lists per view (owner; S2 for the opt-in delta mode at 300 units, S7 for recording-grade positions); the config and announce lines as an internal format (owner, v1.1); the casual policy's missing phase term on **every** fog-filtered method — a defect on `main`, fix named in item 107, not taken here (owner; hardening or T20); what `_status` shows for an untimed Lull and for a recap (owner, S1).
...
**17. Who owns the mesher's input type.** *(Corrected by item 106: the client has no edge to the sim, so the conversion `client-gdext` performs is from the WIRE’s chunk bytes — `get_view`’s run-length runs — not from a borrowed sim view.)* *(T4, wave 1/2.)* Item 56 says the mesher takes "integer
